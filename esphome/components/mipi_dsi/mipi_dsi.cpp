...
void MipiDsi::setup() {
  ...
  esp_lcd_dsi_bus_config_t bus_config = {
      ...
      // LEFT UNSET ON PURPOSE (0 = "not assigned"), so esp_lcd_new_dsi_bus()
      // picks the source that matches the silicon revision.
      //
      // Upstream passes MIPI_DSI_PHY_CLK_SRC_DEFAULT, which is a backward
      // compatibility alias for MIPI_DSI_PHY_PLLREF_CLK_SRC_DEFAULT_LEGACY =
      // PLL_F20M. clk_tree_defs.h marks that source "only available on
      // esp32p4 version < 3.0"; on a rev >= 3.0 chip the switch in
      // mipi_dsi_ll.h has no case for it and calls abort(). On a Waveshare
      // ESP32-P4 (rev v3.2) that is an instant, message-less panic inside
      // setup() -- the board simply never shows anything.
      //
      // With 0 the driver applies its own revision-aware default (XTAL on
      // rev >= 3.0, PLL_F20M below that), which is what Espressif's and
      // Waveshare's own BSPs rely on.
